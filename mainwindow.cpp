...
void MainWindow::Insert_Data(QString DataWHO){
     ...
     QSqlQuery query(db);
     query.prepare(tr("UPDATE SMA_ITX_M_REPEATER set LAST_ACTION =NOW() WHERE REPEATER='%1'").arg(WhoInput.at(2)));
     query.exec();
}
...
void MainWindow::add_RFaddress(QString RF_Address){
    ...
    query.prepare(tr("insert into sma_itx_m_device_list (rf_address,plant_id,line_id,process_id,zone_id,subzone_id,add_date) values('%1','1','1','1',1,1,current_timestamp)").arg(RF_Address));
    query.exec();
    isi_comboBoxGateway();
    initTable_Simulasi();
}
...
void MainWindow::SqlserverDB(){
    ...
    QString Conn = QString("DRIVER={SQL Server};Server=%1;Database=%2;Uid=%3;Port=80;Pwd=%4;WSID=.").arg("127.0.0.1").arg("data_andon").arg("sa").arg("asdf1234");
...
void MainWindow::panggil_parameter(){
    QString Loc;
    QString TipeDB_STR,Servername_STR,Username_STR,Password_STR,DBName_STR,Select_RepeaterSTR;//database
    ...
    if(setting_ip.open(QIODevice::ReadWrite)){
       ...
       QString text = stream.readAll();
        ...
        qDebug()<<Servername_STR<<"#"<<Username_STR<<"#"<<Password_STR<<"#"<<DBName_STR<<"#"<<Select_RepeaterSTR<<"#"<<endl;
}
...
void MainWindow::Save_Database_ini(){
    ...
    QString Tipe_DB,Servername_STR,Username_STR,Password_STR,DBName_STR,Select_Repeater;
    QString Loc;
    Loc=qApp->applicationDirPath();
    ...
    if(setting_ip.open(QIODevice::ReadWrite)){
        QTextStream stream(&setting_ip);
        stream<<Tipe_DB<<"#"<<Servername_STR<<"#"<<Username_STR<<"#"<<Password_STR<<"#"<<DBName_STR<<"#"<<Select_Repeater<<"#"<<endl;
    }
    setting_ip.close();
}
